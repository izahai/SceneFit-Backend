...
Generate descriptions from a background image, then rank clothes using the captions cache (optional feedback text can refine results).
##### POST
`multipart/form-data`
```js
{
	image: "[image file: png/jpg]",
	fb_text: "[optional feedback text]"
}
```
returns
```js
